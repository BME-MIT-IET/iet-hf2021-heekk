...
const ns = (typeof module !== "undefined" && module.exports ? module : window);
...
var isIe = !isNode && ns.navigator && ns.navigator.userAgent && ~ns.navigator.userAgent.indexOf("MSIE");
...
function getCurrUrl() {
    if (isNode) {
        if (!_currNodeUrl) {
            _currNodeUrl =
                "file://" + (process.platform.match(RX_WIN) ? "/" : "") + nodeRequire("fs").realpathSync(".");
        }
        return _currNodeUrl;
...
function parse(self, url, absolutize) {
    ...
    self = updatePath(self, config.protocol, absolutize, url);
...
function updatePath(self, protocol, absolutize, url) {
    if (!protocol && absolutize) { // is IE and path is relative
