...
describe('Special characters in path', function () {
    it('Should encode special characters', function () {
        var url = new Url("http://google.com/a áéöőíüóúű");
    ...
    it('Should decode special characters', function () {
        var url = new Url("http://google.com/%E2%80%A6");
        expect(url.decode(url.toString())).to.be.equal('http://google.com/…');
    });

    it('Should not decode invalid character 3 hex codes', function () {
        var url = new Url("http://google.com/%E0%9f%A6");
        expect(url.decode(url.toString())).to.be.equal('http://google.com/%E0%9f%A6');
    });

    it('Should not decode invalid character 2 hex codes', function () {
        var url = new Url("http://google.com/%C1%9f");
        expect(url.decode(url.toString())).to.be.equal('http://google.com/%C1%9f');
    });
});
